module Yoga.React.R3F.Three.Core where
...
import Yoga.React.R3F.Three.Types (BufferAttribute, BufferGeometry, Clock, Matrix4, Object3D, PlaneGeometry, Vector3)
import Unsafe.Coerce (unsafeCoerce)

class Object3DOps a where
...
instance Object3DOps Object3D where
...
instance Object3DOps (Ref JSX) where
...
class BufferGeometryOps a where
...
instance BufferGeometryOps BufferGeometry where
...
instance BufferGeometryOps PlaneGeometry where
  getIndex = runEffectFn1 bufferGeoGetIndex <<< coercePlane
  getAttribute = runEffectFn2 bufferGeoGetAttribute <<< coercePlane
  setAttribute = runEffectFn3 bufferGeoSetAttribute <<< coercePlane
  translate = runEffectFn4 bufferGeoTranslate <<< coercePlane
  lookAt = runEffectFn2 bufferGeoLookAt <<< coercePlane

coercePlane :: PlaneGeometry -> BufferGeometry
coercePlane = unsafeCoerce

class ClockOps a where
  getElapsedTime :: a -> Effect Number

instance ClockOps Clock where
...
instancedBufferGeometry :: forall props. { | props } -> JSX
